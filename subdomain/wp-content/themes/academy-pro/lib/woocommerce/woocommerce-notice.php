...
function academy_woocommerce_theme_notice() {
	...
	echo '<div class="notice notice-info is-dismissible academy-woocommerce-notice"><p>' . wp_kses_post( $notice_html ) . '</p></div>';
...
function academy_notice_script() {
	...
	wp_enqueue_script( 'academy_notice_script', get_stylesheet_directory_uri() . '/lib/woocommerce/js/notice-update.js', [ 'jquery' ], '1.0', true );
...
function academy_reset_woocommerce_notice() {
	...
	$args = [
		'meta_key'   => $wpdb->prefix . 'academy_woocommerce_message_dismissed', // phpcs:ignore WordPress.DB.SlowDBQuery.slow_db_query_meta_key
