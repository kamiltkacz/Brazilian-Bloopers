...
function author_woocommerce_theme_notice() {
	...
	if ( current_user_can( 'install_plugins' ) ) {
		...
		$notice_html = sprintf( __( 'Please %1$s to <strong>enable WooCommerce support for %2$s</strong>.', 'author-pro' ), $install_link, esc_html( CHILD_THEME_NAME ) );
	}

	echo '<div class="notice notice-info is-dismissible author-woocommerce-notice"><p>' . wp_kses_post( $notice_html ) . '</p></div>';
...
function author_notice_script() {
	wp_enqueue_script( 'author_notice_script', get_stylesheet_directory_uri() . '/lib/woocommerce/js/notice-update.js', [ 'jquery' ], '1.0', true );
}
...
function author_reset_woocommerce_notice() {
	...
	$args  = [
		'meta_key'   => $wpdb->prefix . 'author_woocommerce_message_dismissed', // phpcs:ignore WordPress.DB.SlowDBQuery.slow_db_query_meta_key
 ...
 */
function author_reset_woocommerce_notice_on_deactivation( $plugin, $network_deactivating ) {
